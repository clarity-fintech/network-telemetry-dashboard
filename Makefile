...
check:
	...
	$(MISC) clrty/network-telemetry-dashboard.mis --check --compact-letters
	$(MISC) mis/clrty1_e2e_bind.mis --check --compact-letters
	$(MISC) mis/kernel/NetworkTelemetryDashboardKernel.mis --check --compact-letters
	@find mis -name '*.mis' | while read f; do $(MISC) "$$f" --check --compact-letters || exit 1; done
	@echo OK network-telemetry-dashboard
...
network-connect: commands-check
	...
	@echo MODULES mis/commands/NetworkTelemetryDashboardCommands.mis
